...
const SORT_OPTIONS = [
  ...
  },
  {
    key: "followers_desc",
    label: "FOLLOWERS",
    fn: (a, b) => (b.followers || 0) - (a.followers || 0),
  },
  {
    key: "activity_desc",
    label: "ACTIVITY",
    fn: (a, b) => (b.events_30d || 0) - (a.events_30d || 0),
  },
];
...
export default function Leaderboard({
  ...
  useEffect(() => {
    ...
    async function loadAll() {
      ...
      try {
        ...
        if (!leaderboardPayload) {
          throw new Error(
            "Failed to load data.json and no cached data available.",
          );
        }
  ...
  const tags = useMemo(
    () => ["All", ...getAvailableTags(leaderboard)],
    [leaderboard],
  );
  ...
  const filteredLeaderboard = useMemo(() => {
    ...
    if (selectedTag !== "All") {
      result = result.filter(
        (dev) => Array.isArray(dev?.tags) && dev.tags.includes(selectedTag),
      );
    }
    ...
    if (q) {
      result = result.filter((dev) => {
        ...
        const cleanLocation = normalizeLocationForDisplay(
          dev.location,
        ).toLowerCase();
  ...
  useEffect(() => {
    setCurrentPage(1);
  }, [selectedTag, searchTerm, sortIndex]);

  const indexOfLastDev = currentPage * devsPerPage;
  const indexOfFirstDev = Math.max(0, indexOfLastDev - devsPerPage);
  const currentDevs = filteredLeaderboard.slice(
    indexOfFirstDev,
    indexOfLastDev,
  );
  const totalPages = Math.max(
    1,
    Math.ceil(filteredLeaderboard.length / devsPerPage),
  );
  ...
  return (
    <main className="min-h-screen relative overflow-hidden">
      ...
      <div className="max-w-7xl mx-auto p-4 md:p-8 relative z-10">
        ...
        {/* ── Badge Banner ── */}
        {onChangeTab && (
          <BadgeBanner onNavigateToBadge={() => onChangeTab("badge")} />
        )}
            ...
            {filteredLeaderboard.length > 0 && (
              <div className="mt-8 flex flex-col md:flex-row justify-between items-center gap-4 font-mono text-xs border border-outline-variant p-4 bg-surface-container-lowest">
                <div className="text-outline uppercase">
                  Showing {String(startIdx).padStart(3, "0")} -{" "}
                  {String(endIdx).padStart(3, "0")} of{" "}
                  {filteredLeaderboard.length.toLocaleString()} Node_Instances
                </div>
